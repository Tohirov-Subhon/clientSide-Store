    ...
    return (
      <div className="flex p-8 bg-gray-100 min-h-screen">
        <div className="flex flex-col space-y-4">
          {data?.images?.map((el) => (
            <img onClick={() => setSelectImage(el.images)} key={el.id} src={api+'/images/' + el.images} alt="Thumbnail" className="w-24 h-24 object-cover rounded-lg cursor-pointer border" />
          ))}
        </div>
        ...
        <div className="flex flex-col md:flex-row md:space-x-8 bg-white p-6 rounded-lg shadow-lg w-full">
          <img  src={api+'/images/' + selctImage} alt="Controller" className="w-1/2 object-cover"  />
